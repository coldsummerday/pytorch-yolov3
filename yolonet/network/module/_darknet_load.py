...
__all__=["DarknetLoader"]

class DarknetLoader(nn.Module):
    ...
    def save_weights(self, weights_file, seen=None):
        ...
        if os.path.splitext(weights_file)[1] == '.pt':
            ...
            self._save_weights(weights_file, seen)
        else:
            log.debug('Saving weights to darknet file')
            self._save_darknet_weights(weights_file, seen)

    def _save_pt_weights(self, weights_file, seen=None):
        """ This function will save the weights to a file.
    ...
    def load_weights(self,weights_file,clear=False):
        ...
        if weights_file.endswith(".pt"):
            ...
            self._load_pt_weights(weights_file,clear)
        else:
    ...
    def _load_pt_weights(self,weights_file,clear=False):
        """ This function will load the weights from a file.
    ...
    def _load_darknet_weights(self,weights_file,clear=False):
        weights_handler = WeightLoader(weights_file)
        self.header = weights_handler.header
        if clear:
            self.seen = 0
        if hasattr(self.loss, 'seen'):
            self.loss.seen = self.seen
        for index,module in enumerate(self.modules_recurse()):

            try:

                weights_handler.load_layer(module)
                log.debug(f'{index} Layer loaded: {module}')
                if weights_handler.start>= weights_handler.size:
                    log.debug(f'Finished loading weights [{weights_handler.start}/{weights_handler.size} weights]')
                    break
            except NotImplementedError:
                log.debug(f'Layer skipped: {module.__class__.__name__}')


    def _save_darknet_weights(self, weights_file,clear=False):
        if hasattr(self.loss, 'seen') and not clear:
...
class WeightLoader:
    ...
    def load_layer(self, layer):
        ...
        elif type(layer) == net_layer.Conv2dBatchLeaky:
